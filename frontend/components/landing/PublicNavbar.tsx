...
export default function PublicNavbar() {
  ...
  return (
    <header className="landing-navbar sticky top-0 z-30 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/90">
      ...
      {open && <div className="landing-mobile-menu border-t border-slate-200 bg-white px-5 py-4 dark:border-slate-800 dark:bg-slate-950 sm:hidden"><div className="flex flex-col gap-4"><Link href="/" onClick={() => setOpen(false)} className="text-sm text-slate-700 dark:text-slate-200">Home</Link><Link href="#explore" onClick={() => setOpen(false)} className="text-sm text-slate-700 dark:text-slate-200">Explore</Link><Link href="#categories" onClick={() => setOpen(false)} className="text-sm text-slate-700 dark:text-slate-200">Categories</Link><ThemeToggle />{isAuthenticated ? <Link href="/dashboard/reader" className="inline-flex items-center gap-2 text-sm font-semibold text-violet-600 dark:text-violet-400"><LayoutDashboard className="h-4 w-4" /> Dashboard</Link> : <div className="flex gap-3"><Link href="/login" className="text-sm text-slate-600 dark:text-slate-300">Sign in</Link><Link href="/register" className="text-sm font-semibold text-violet-600 dark:text-violet-400">Get started</Link></div>}</div></div>}
    </header>
  );
}

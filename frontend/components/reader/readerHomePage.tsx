  ...
  return (
    ...
    >
      ...
      <section className="grid gap-4 lg:grid-cols-[1.4fr_0.6fr]">
        {continueBlog ? (
          <Link href={`/dashboard/reader/blogs/${continueBlog._id}`} className="reader-continue-card group flex items-center gap-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-5 transition hover:-translate-y-0.5 hover:border-emerald-300 dark:border-emerald-800/50 dark:bg-emerald-950/30">
            <span className="reader-continue-icon flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-emerald-500/15 text-emerald-700 dark:text-emerald-400"><History className="h-5 w-5" /></span>
            <div className="min-w-0"><p className="text-xs font-semibold uppercase tracking-[0.15em] text-emerald-700 dark:text-emerald-400">Continue reading</p><h2 className="mt-1 truncate text-lg font-semibold text-slate-900 dark:text-emerald-100">{continueBlog.title}</h2><p className="mt-1 text-sm text-slate-700 dark:text-emerald-300/80">Pick up where you left off <ArrowUpRight className="ml-1 inline h-4 w-4 transition group-hover:translate-x-0.5" /></p></div>
          </Link>
        ...
        <div className="reader-streak-card flex items-center gap-4 rounded-2xl border border-orange-200 bg-orange-50 p-5 dark:border-orange-800/50 dark:bg-orange-950/30"><span className="reader-streak-icon flex h-11 w-11 items-center justify-center rounded-xl bg-orange-500/15 text-orange-700 dark:text-orange-400"><Flame className="h-5 w-5" /></span><div><p className="text-2xl font-bold text-slate-900 dark:text-orange-100">{readingStreak} day{readingStreak === 1 ? "" : "s"}</p><p className="text-sm font-semibold text-slate-900 dark:text-orange-100">Reading streak</p><p className="text-sm text-slate-700 dark:text-orange-300/80">{readingStreak ? "Keep the momentum going" : "Read one story to begin"}</p></div></div>
      </section>
